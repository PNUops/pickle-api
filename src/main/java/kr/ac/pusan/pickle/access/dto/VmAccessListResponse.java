 ...
 * ones who cannot open the VM itself: an owner of the workspace holds no grant, so
 ...
 * state, owning workspace — and nothing from inside.
 */
...
public record VmAccessListResponse(
    ...
    public static VmAccessListResponse of(kr.ac.pusan.pickle.vm.Vm vm, String workspaceName,
            @Nullable String displayName, List<VmAccessGrantView> grants) {
        return new VmAccessListResponse(
                new Vm(vm.getId(), vm.getName(), displayName, vm.getStatus(), vm.getWorkspaceId(),
                        workspaceName),
                grants);
    }
}
